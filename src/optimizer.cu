...
void Optimizer::optimizeTest(TSDFVolume* tsdfLive)
{
    ...
    */
    ...
    do
    {
		...
		/*float* tsdfLiveDeformTest = new float[m_gridW * m_gridH * m_gridD];
        float* tsdfGlobalTest = new float[m_gridW * m_gridH * m_gridD];
        float* deformUTest = new float[m_gridW * m_gridH * m_gridD];
        float* deformVTest = new float[m_gridW * m_gridH * m_gridD];
        float* deformWTest = new float[m_gridW * m_gridH * m_gridD];
        cudaMemcpy(tsdfLiveDeformTest, m_d_tsdfLiveDeform, (m_gridW * m_gridH * m_gridD) * sizeof(float), cudaMemcpyDeviceToHost); CUDA_CHECK;
        cudaMemcpy(tsdfGlobalTest, m_d_tsdfGlobal, (m_gridW * m_gridH * m_gridD) * sizeof(float), cudaMemcpyDeviceToHost); CUDA_CHECK;
        cudaMemcpy(deformUTest, m_d_deformationFieldU, (m_gridW * m_gridH * m_gridD) * sizeof(float), cudaMemcpyDeviceToHost); CUDA_CHECK;
        cudaMemcpy(deformVTest, m_d_deformationFieldV, (m_gridW * m_gridH * m_gridD) * sizeof(float), cudaMemcpyDeviceToHost); CUDA_CHECK;
        cudaMemcpy(deformWTest, m_d_deformationFieldW, (m_gridW * m_gridH * m_gridD) * sizeof(float), cudaMemcpyDeviceToHost); CUDA_CHECK;
        for (int i = 0; i < m_gridW * m_gridH * m_gridD; i++)
        {
			if (tsdfGlobalTest[i] != tsdfLiveDeformTest[i])
			{
				std::cout << "TSDF Global[" << i << "]: " << tsdfGlobalTest[i] << " TSDF Live: " << tsdfLiveDeformTest[i] << std::endl;
				std::cout << "   -> Deformation (u, v, w): " << deformUTest[i] << " " << deformVTest[i] << " " << deformWTest[i] << std::endl;
				std::cout << "   -> Initial Deformation (u, v, w): " << m_deformationFieldU[i] << " " << m_deformationFieldV[i] << " " << m_deformationFieldW[i] << std::endl;
			}
        }*/



        // Test that tsdfLiveDeform is the same as tsdfGlobal
        /*float* tsdfLiveDeformTest = new float[m_gridW * m_gridH * m_gridD];
        float* tsdfGlobalTest = new float[m_gridW * m_gridH * m_gridD];
        float* deformUTest = new float[m_gridW * m_gridH * m_gridD];
        float* deformVTest = new float[m_gridW * m_gridH * m_gridD];
        ...
        for (int i = 0; i < m_gridW * m_gridH * m_gridD; i++)
        {
            if (tsdfGlobalTest[i] != tsdfLiveDeformTest[i])
            {
                std::cout << "TSDF Global: " << tsdfGlobalTest[i] << " TSDF Live Deform: " << tsdfLiveDeformTest[i] << std::endl;
                std::cout << "   -> Deformation (u, v, w): " << deformUTest[i] << " " << deformVTest[i] << " " << deformWTest[i] << std::endl;
                std::cout << "   -> Initial Deformation (u, v, w): " << m_deformationFieldU[i] << " " << m_deformationFieldV[i] << " " << m_deformationFieldW[i] << std::endl;
            }
        }*/
